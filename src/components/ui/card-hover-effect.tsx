...
export interface HoverEffectCardHeaderProps extends PropsWithClass {
  icon?: React.ReactNode;
  items?: { link: string; icon?: React.ReactNode }[];
}

export interface HoverEffectProps extends PropsWithClass {
  items: {
    title: string;
    description: string;
...
export interface HoverEffectItemProps extends PropsWithClass {
  item: {
    title: string;
    description: string;
    header?: HoverEffectCardHeaderProps;
  ...
  return (
    ...
    >
      <AnimatePresence>
        {hoveredIndex === index && (
          <motion.span
            className="bg-tertiary absolute inset-0 block size-full rounded-3xl bg-opacity-40"
...
export const Card = ({ className, children }: PropsWithClass) => (
  <div
    className={cn(
...
export const CardTitle = ({ className, children }: PropsWithClass) => (
  <h4 className={cn('text-2xl font-bold tracking-wide text-primary', className)}>{children}</h4>
);

export const CardDescription = ({ className, children }: PropsWithClass) => (
